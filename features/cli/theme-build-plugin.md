...
- directives select the layout injection point only; Shopify section group JSON remains Shopify/admin-owned and is not generated by the plugin
- invalid values are build errors
...
Shopify section group JSON files (`sections/header-group.json` and `sections/footer-group.json`) are scaffold/admin-owned, not Vite-generated. Merchants may add, remove, configure, and reorder header/footer group sections in Shopify admin without the build overwriting that state.
...
- plugin leaves Shopify section group JSON untouched
...
- [ ] does not generate or overwrite sections/header-group.json or sections/footer-group.json
- [ ] in watch mode, editing layout/theme.source.liquid triggers a rebuild
...
In watch mode, the `options` hook (called by Rollup before every rebuild) regenerates theme files and updates the Rollup input so new sections are compiled without a process restart. The `buildStart` hook watches the `sections/`, `snippets/`, `config/`, and `layout/` directories so file additions trigger the next rebuild, and individual liquid and settings files so edits trigger rebuilds.
