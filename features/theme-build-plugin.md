...
invariants:
  - Plugin code lives in this repo as an internal module
...
codebaseOwnership:
  owns:
    repo:
      - internal Nazare Vite plugin module source
...
### Internal module location

Plugin code should live in this repo as an internal module.

Initial expected shape:

```txt
src/theme-build-plugin/index.js
src/theme-build-plugin/scanner.js
src/theme-build-plugin/generator.js
src/theme-build-plugin/runtime-template.js
```

Package/export wiring can be refined when scaffold `vite.config.js` imports the plugin.
...
The plugin is an internal module first. A public package/export boundary can be introduced when external registry/theme consumption needs it.
...
- What exact package export path should scaffold `vite.config.js` use for the internal plugin?
